...
const projects = [
  {
    ...
    types: ['AI ORCHESTRATION', 'PYTHON', 'GEMINI LLM', 'FLASK'],
    color: 'bg-purple-300',
    details: [
...
const moveSets = [
  { type: 'STEEL / ELECTRIC', color: 'bg-amber-400', title: 'LOW-LEVEL & SYSTEMS', moves: ['C', 'C++', 'Embedded C++', 'Low-Latency Execution', 'Data Structures & Algorithms'] },
  { type: 'PSYCHIC', color: 'bg-purple-300', title: 'AI & MACHINE LEARNING', moves: ['PyTorch', 'Gemini LLM / Google ADK', 'Multi-Agent Orchestration', 'Scikit-Learn', 'Pandas', 'NumPy', 'RAG Architectures'] },
  { type: 'WATER / GRASS', color: 'bg-cyan-400', title: 'WEB & CLOUD ENGINEERING', moves: ['React', 'TypeScript', 'JavaScript', 'Next.js', 'Node.js', 'Express', 'HTML/CSS'] },
  { type: 'GROUND / ROCK', color: 'bg-emerald-400', title: 'DATABASES & DEVOPS', moves: ['SQL', 'PostgreSQL', 'MySQL', 'MongoDB', 'RESTful APIs', 'Flask', 'GitHub Actions CI/CD', 'AWS'] },
]

const quests = [
  { title: 'Software Engineer Intern', org: 'Dimelco S.A.S', location: 'Cúcuta, Colombia', duration: 'May 2024 - Aug 2024', reward: 'WEB + CLOUD EXP', color: 'bg-amber-400', details: ['Developed prototype landing pages and employee management UI with JS, HTML, CSS, React, and TypeScript.', 'Engineered Python REST APIs and SQL queries validated through GitHub Actions CI/CD on AWS.', 'Leveraged LLMs and agentic AI workflows under senior engineering mentorship.'] },
  { title: 'Teaching Assistant', org: 'University of Central Florida', location: 'Orlando, FL', duration: 'May 2025 - Present', reward: '200+ STUDENTS / TERM', color: 'bg-cyan-400', details: ['Mentoring 200+ students per semester in Data Structures, Algorithms, and Object-Oriented Programming using C and Java.', 'Designing supplementary lab exercises, code-debugging workshops, and exam review materials.'] },
  { title: 'Graduate Admissions Board', org: 'Tau Beta Pi Honor Society', location: 'Orlando, FL', duration: 'Aug 2025 - Dec 2025', reward: 'OPS OPTIMIZATION', color: 'bg-purple-300', details: ['Optimized admissions workflows for incoming graduate engineering candidates.'] },
]
...
function PixelAvatar() {
  return (
    <div className="relative flex flex-col items-center gap-4 overflow-hidden border-4 border-slate-900 bg-cyan-300 p-4 shadow-[6px_6px_0px_0px_#38405f] sm:p-5">
      ...
      <div className="relative z-10 self-start font-pixel text-[9px]">TRAINER SPRITE</div>
      ...
      <div className="relative z-10 w-full space-y-2 font-pixel text-[8px] leading-relaxed">
        <div className="border-2 border-slate-900 bg-slate-50 px-2 py-1.5">STATUS: BUILDING AI + LOW-LATENCY SYSTEMS</div>
...
function PokedexCard() {
  ...
  return (
    <section id="projects" className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <SectionTitle kicker="PROJECTS">PROJECT SHOWCASE</SectionTitle>
      <div className="border-4 border-slate-900 bg-rose-400 p-3 shadow-[8px_8px_0px_0px_#0f172a] sm:p-5">
        ...
        <div className="grid gap-5 lg:grid-cols-[230px_1fr]">
          <nav className="grid grid-cols-2 gap-2 lg:grid-cols-1" aria-label="Project selector">
            {filteredProjects.map(item => <button key={item.title} onClick={() => setActiveTitle(item.title)} className={`${hardButton} px-3 py-3 text-left font-pixel text-[9px] ${project.title === item.title ? 'bg-slate-900 text-slate-50' : 'bg-slate-50 text-slate-900'}`}><span className="mr-2 text-amber-400">{String(projects.findIndex(project => project.title === item.title) + 1).padStart(2, '0')}</span>{item.title}</button>)}
          </nav>
          <article className="min-h-[375px] border-4 border-slate-900 bg-slate-50 p-5 sm:p-7">
            <div className="flex flex-wrap items-start justify-between gap-4 border-b-4 border-slate-900 pb-5"><div><p className="font-pixel text-[9px] text-slate-600">ENTRY_{String(projectNumber).padStart(3, '0')} | {project.category}</p><h3 className="mt-2 font-pixel text-2xl leading-relaxed sm:text-3xl">{project.title}</h3><p className="mt-2 font-body font-bold">{project.date}</p></div><div className="pokedex-orb shrink-0" aria-hidden="true" /></div>
            <div className="mt-5 flex flex-wrap gap-2">{project.types.map(type => <span key={type} className={`border-2 border-slate-900 ${project.color} px-2 py-1 font-pixel text-[8px]`}>{type}</span>)}</div>
...
function MoveSet() { return <section id="skills" className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8"><SectionTitle kicker="[SECTION 03]">MOVE-SET & STATS</SectionTitle><div className="grid gap-5 md:grid-cols-2">{moveSets.map((set, index) => <article key={set.title} className="border-4 border-slate-900 bg-slate-50 shadow-[5px_5px_0px_0px_#0f172a]"><div className={`${set.color} border-b-4 border-slate-900 p-4`}><div className="flex items-center justify-between gap-2"><h3 className="font-pixel text-sm leading-relaxed">{set.title}</h3><span className="border-2 border-slate-900 bg-slate-50 px-2 py-1 font-pixel text-[8px]">{set.type}</span></div></div><div className="p-4"><p className="mb-3 font-pixel text-[9px] text-slate-600">MOVE BANK_{String(index + 1).padStart(2, '0')}</p><div className="flex flex-wrap gap-2">{set.moves.map(move => <span key={move} className="border-2 border-slate-900 bg-slate-100 px-2 py-1.5 font-body text-sm font-bold transition-transform hover:-translate-y-0.5">{move}</span>)}</div></div></article>)}</div></section> }

function QuestLog() { const [active, setActive] = useState(0); const quest = quests[active]; return <section id="quests" className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8"><SectionTitle kicker="[SECTION 04]">QUEST LOG</SectionTitle><div className="grid gap-5 lg:grid-cols-[.75fr_1.25fr]"><div className="space-y-3">{quests.map((item, index) => <button key={item.org} onClick={() => setActive(index)} className={`${hardButton} w-full p-4 text-left ${active === index ? 'bg-slate-900 text-slate-50' : 'bg-slate-50 text-slate-900'}`}><p className="font-pixel text-[9px]">QUEST_{String(index + 1).padStart(2, '0')}</p><p className="mt-2 font-pixel text-sm leading-relaxed">{item.title}</p><p className="mt-1 font-body text-sm font-bold">@ {item.org}</p></button>)}</div><article className="border-4 border-slate-900 bg-slate-50 p-5 shadow-[6px_6px_0px_0px_#0f172a] sm:p-7"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="font-pixel text-[10px] text-slate-600">ACTIVE QUEST</p><h3 className="mt-2 font-pixel text-xl leading-relaxed">{quest.title}</h3><p className="mt-2 font-body font-bold">{quest.org} · {quest.location}</p></div><span className={`${quest.color} border-4 border-slate-900 px-3 py-2 font-pixel text-[8px]`}>REWARD: {quest.reward}</span></div><div className="mt-6 border-y-4 border-slate-900 py-3 font-pixel text-[9px]">DURATION: {quest.duration}</div><ul className="mt-6 space-y-4 font-body text-base leading-7">{quest.details.map(detail => <li key={detail} className="flex gap-3"><span className="mt-2 h-3 w-3 shrink-0 border-2 border-slate-900 bg-amber-400" />{detail}</li>)}</ul></article></div></section> }
...
function App() {
  return <main className="city-main min-h-screen overflow-x-hidden text-slate-900"><PixelSkyline /><div className="site-content relative z-10"><header className="border-b-4 border-slate-900 bg-slate-50"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-3 font-pixel text-[9px] sm:px-6 lg:px-8"><span className="border-2 border-slate-900 bg-purple-300 px-2 py-1.5">#XAVIER_SOTO_V2.6 // EN-ES</span><span>LVL 2027 <i className="mx-2 not-italic text-amber-500">|</i> ORLANDO, FL</span></div></header><TrainerCard /><Profile /><PokedexCard /><MoveSet /><QuestLog /><Footer /></div></main>
}
