...
function PokemonParty() {
  return <aside className="border-4 border-slate-900 bg-slate-50 p-3 shadow-[5px_5px_0px_0px_#38405f]" aria-label="Xavier's PokÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©mon companion party">
...
function LegacyPokedexCard() {
  ...
  return <section id="projects" className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8"><SectionTitle kicker="[SECTION 02]">POKÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â°DEX PROJECT SHOWCASE</SectionTitle><div className="border-4 border-slate-900 bg-rose-400 p-3 shadow-[8px_8px_0px_0px_#0f172a] sm:p-5"><div className="grid gap-5 lg:grid-cols-[230px_1fr]">
      ...
      <div className="border-4 border-slate-900 bg-rose-400 p-3 shadow-[8px_8px_0px_0px_#0f172a] sm:p-5">
        ...
        <div className="grid gap-5 lg:grid-cols-[230px_1fr]">
          ...
          <article id="project-details" className={`${isExpanded ? 'min-h-[590px] sm:p-8' : 'min-h-[375px]'} border-4 border-slate-900 bg-slate-50 p-5 transition-[min-height,padding] duration-300 sm:p-7`}>
            <div className="flex flex-wrap items-start justify-between gap-4 border-b-4 border-slate-900 pb-5">
              <div className={`${project.color} min-w-0 flex flex-1 flex-col justify-between self-stretch border-4 border-slate-900 p-4 shadow-[4px_4px_0px_0px_#38405f] sm:p-5`}><div className="flex flex-wrap items-center gap-x-3 gap-y-1 font-pixel text-[9px] text-slate-600"><span>ENTRY_{String(projectNumber).padStart(3, '0')}</span>{getCategories(project).map(category => <span key={category}>| {category}</span>)}</div><div className="mt-5 mb-2"><div className="border-y-2 border-slate-900 py-3">{project.liveUrl ? <a href={project.liveUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 font-pixel text-3xl leading-relaxed underline decoration-4 underline-offset-4 transition-colors hover:text-[#505775] focus-visible:outline-4 focus-visible:outline-offset-4 focus-visible:outline-slate-900 sm:text-4xl">{project.title}<span aria-hidden="true">{'\u2197'}</span></a> : <h3 className="font-pixel text-3xl leading-relaxed sm:text-4xl">{project.title}</h3>}</div></div><div><p className="py-2 font-body font-bold">{project.purpose}</p><p className="border-t-2 border-slate-900 pt-2 font-body font-bold">{project.date}</p></div></div>
              {project.pictures[0] ? <figure className="project-reference w-full overflow-hidden border-4 border-slate-900 bg-sky-100 sm:w-80 lg:w-[26rem]"><img className="aspect-[16/11] w-full object-cover" src={project.pictures[0].src} alt={project.pictures[0].alt} /></figure> : <div className="flex h-24 w-full items-center justify-center border-4 border-dashed border-slate-900 bg-sky-100 px-3 text-center font-pixel text-[9px] text-slate-600 sm:w-52">PROJECT MEDIA<br />PENDING</div>}
            </div>
            ...
            <div className="mt-7 flex flex-wrap gap-3 border-t-4 border-slate-900 pt-5">{project.pictures.length > 0 && <button type="button" onClick={() => setIsExpanded(expanded => !expanded)} aria-expanded={isExpanded} aria-controls="project-media" className={`${hardButton} bg-amber-400 px-4 py-3 font-pixel text-[10px] text-slate-900`}>{isExpanded ? 'HIDE IMAGES' : `MORE IMAGES (${project.pictures.length})`}</button>}{project.githubUrl ? <a className={`${hardButton} bg-slate-900 px-4 py-3 font-pixel text-[10px] text-slate-50`} href={project.githubUrl} target="_blank" rel="noreferrer">GITHUB</a> : <span className="border-4 border-slate-900 bg-slate-100 px-4 py-3 font-pixel text-[10px] text-slate-600" title="Add a githubUrl to this project to enable the repository link" aria-disabled="true">GITHUB LINK PENDING</span>}</div>
            {isExpanded && project.pictures.length > 0 && <div id="project-media" className="mt-7 border-t-4 border-slate-900 pt-6"><p className="font-pixel text-[10px] text-slate-600">PROJECT GALLERY</p>{project.pictures.length ? <div className="mt-4 grid gap-6 xl:grid-cols-2">{project.pictures.map((picture, index) => <figure key={`${picture.src}-${index}`} className="overflow-hidden border-4 border-slate-900 bg-slate-100"><img className="aspect-[16/11] w-full object-cover" src={picture.src} alt={picture.alt} /></figure>)}</div> : <p className="mt-4 border-2 border-dashed border-slate-900 bg-sky-100 p-4 font-body text-sm leading-6">Screenshots will appear here when picture objects are added to this project.</p>}</div>}
          </article>
        </div>
      </div>
    </section>
